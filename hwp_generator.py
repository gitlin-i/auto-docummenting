...
class HwpGenerator:
    ...
    def get_manager_vacation_days(self, manager_name, year, month, data):
        """해당 매니저의 해당 월 연가 날짜 리스트 반환 (정수 day 리스트)"""
        vacation_schedules = data.get("vacationSchedules", {})
        vacation_days = []
        for date_str, vacation_list in vacation_schedules.items():
            for vacation in vacation_list:
                if vacation.get("name") == manager_name:
    ...
    def add_vacation_lines(self, year, month, last_day, vacation_days):
        """%일1, %일2 헤더 기준으로 각 열을 순회하며 셀 값을 직접 읽어 연가 입력"""
            ...
            for _ in range(max_rows):
                ...
                if isinstance(text_tuple, tuple) and len(text_tuple) > 1:
                    try:
                        ...
                        continue
                if cell_value in vacation_days:
